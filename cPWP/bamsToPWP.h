...
int calcPWPforRange (unsigned long long startingLocus, unsigned long long endingLocus, const std::vector<unsigned char>& mainReadCountVector, std::vector< std::vector<long double> > & threadPWP, std::vector< std::vector<long double> > & threadWeightings);
