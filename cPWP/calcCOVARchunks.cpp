...
int calcCOVARforRange (unsigned long long startingLocus, unsigned long long endingLocus, int numIndividuals, std::vector<unsigned char>& mainReadCountVector, std::vector<std::vector<long double>>& weightSumProducts, std::vector<std::vector<long double>>& weightSumFirst, std::vector<std::vector<unsigned long long int>>& threadWeightings) {
